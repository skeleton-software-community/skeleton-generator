...
public class JsfBaseListControllerFileWriteCommand extends JavaFileWriteCommand {
	...
	protected void fetchSpecificImports() {
		...
		javaImports.add("import " + this.bean.myPackage.filtersPackageName + "." + this.bean.basicViewBean.filter.className + ";");
	...
	private void createSaveObject() {
		...
		if (bean.detailMode.equals(DetailMode.MODAL)) {
			...
			writeLine("redirect(" + CHAR_34 + "/sections/" + bean.myPackage.urlPiece + "/" + this.bean.urlPiece + "/details.jsf?id=" + CHAR_34 + " + result);");
			writeLine("}");
			writeLine("});");
			writeLine("}");
		}
	...
	private void createResetFlters() {
		...
		writeLine("this." + this.bean.listViewObjectName + ".getScrollForm().setFilter(new " + bean.basicViewBean.filter.className + "());");
