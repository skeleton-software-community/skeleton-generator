...
public class Richfaces4OneToManyComponentListViewFileWriteCommand extends Richfaces4XhtmlFileWriteCommand {

	private OneToManyComponent oneToManyComponent;

	public Richfaces4OneToManyComponentListViewFileWriteCommand(OneToManyComponent oneToManyComponent) {
	...
	protected void writeContent() throws IOException {
		...
		writeLine("<a4j:commandLink action=\"#{" + parentBean.detailControllerObjectName + ".reset" + currentBean.className + "Filter}\" render=\"" +  currentBean.objectName + "PanelGroup\">");
		...
		writeLine("</f:facet>");
		writeLine("<h:selectBooleanCheckbox  rendered=\"#{" + currentBean.objectName + ".canDelete}\" id=\"selectUnselect\" value=\"#{" + currentBean.objectName + ".selected}\" onclick=\""
				+ "selectUnselect('" + currentBean.objectName + "ListForm:" + currentBean.objectName + "List:selectUnselectAll')\"/>");
		writeLine("</rich:column>");
		skipLine();
		...
		if (currentBean.deleteEnabled) {
			writeLine("<a4j:commandLink title=\"#{i18n.deleteSelection}\" action=\"#{" + parentBean.detailControllerObjectName + ".delete" + currentBean.className + "List}\"");
			writeLine("onclick=\"if (!confirm('#{i18n.confirmDeleteSelection}')) return false\" execute=\"@region\" render=\"" +  currentBean.objectName + "PanelGroup\">");
		...
		if (currentBean.detailMode.equals(DetailMode.PAGE)) {
			...
			writeLine("<a4j:commandLink action=\"#{" + parentBean.detailControllerObjectName + ".edit" + currentBean.className + "(" + currentBean.objectName + ".id)}\""
		...
		if (currentBean.deleteEnabled) {
			writeLine("<a4j:commandLink action=\"#{" + parentBean.detailControllerObjectName + ".delete" + currentBean.className + "(" + currentBean.objectName + ".id)}\"");
			writeLine("rendered=\"#{" + currentBean.objectName + ".canDelete}\"");
			writeLine("onclick=\"if (!confirm('#{i18n.confirmDelete}')) return false\" execute=\"@this\" render=\"" +  currentBean.objectName + "PanelGroup\">");
		...
		for (ViewProperty property : currentBean.basicViewBean.properties) {
			...
			writeLine("render=\"" +  currentBean.objectName + "PanelGroup\"/>");
		...
		if (currentBean.createEnabled) {
			writeLine("<a4j:commandButton value=\"#{i18n.create}\" action=\"#{" + parentBean.detailControllerObjectName + ".create" + currentBean.className
					...
					+ " execute=\"@this\" render=\"" +  currentBean.objectName + "DetailPanelGroup\"/>");
			skipLine();
		}
