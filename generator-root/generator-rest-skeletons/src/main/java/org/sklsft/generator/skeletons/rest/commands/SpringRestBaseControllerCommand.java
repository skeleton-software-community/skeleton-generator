...
private Bean bean;
	...
	private void createScrollOneToManyComponent() {
		for (OneToManyComponent oneToManyComponent : this.bean.oneToManyComponentList) {
			...
			writeLine("public @ResponseBody ScrollView<" + currentBean.basicViewBean.className + "> scroll" + currentBean.className + " (@PathVariable(\"" + bean.objectName + "Id\") " + bean.idType + " " + bean.objectName + "Id, @RequestBody ScrollForm<" + currentBean.basicViewBean.filterClassName + ", " + currentBean.basicViewBean.sortingClassName + "> form) {");
	...
	private void createSaveObject() {
		...
		writeLine("public @ResponseBody " + bean.idType + " save(@RequestBody " + this.bean.formBean.className + " form) {");
		...
		for (Property property:bean.properties) {
			if (property.referenceBean!=null) {
				if (property.relation.equals(RelationType.MANY_TO_ONE)) {
					...
					writeLine("public @ResponseBody " + bean.idType + " saveFrom" + parentBean.className + "(@PathVariable(\"" + parentBean.objectName + "Id\") " + parentBean.idType + " " + parentBean.objectName + "Id, @RequestBody " + this.bean.formBean.className + " form) {");
	...
	private void createSaveOneToOneComponent() {
		for (OneToOneComponent oneToOneComponent : this.bean.oneToOneComponentList) {
			...
			writeLine("public @ResponseBody void save" + currentBean.className + "(@PathVariable(\"id\") " + bean.idType + " id, @RequestBody " + currentBean.formBean.className + " form) {");
	...
	private void createUpdateObject() {
		...
		writeLine("public void update(@PathVariable(\"id\") " + bean.idType + " id, @RequestBody " + this.bean.formBean.className + " form) {");
	...
	private void createUpdateUniqueComponent() {
		for (OneToOneComponent oneToOneComponent : this.bean.oneToOneComponentList) {
			...
			writeLine("public void update" + currentBean.className + "(" + bean.idType + " id, " + currentBean.formBean.className + " form) {");
	...
	private void createUpdateOneToManyComponent() {
		for (OneToManyComponent oneToManyComponent : this.bean.oneToManyComponentList) {
			...
			writeLine(" */");
			writeLine("@RequestMapping(value = {" + bean.serviceInterfaceName + ".UPDATE_" + currentBean.table.originalName + "_URL}, method = RequestMethod.PUT)");
			writeLine("public void update" + currentBean.className + "(" + currentBean.idType + " id, " + currentBean.formBean.className + " form) {");
			writeLine(bean.serviceObjectName + ".update" + currentBean.className + "(id, form);");
			writeLine("}");
			skipLine();
		}
	...
	private void createDeleteObject() {
		...
		writeLine("public void delete(@PathVariable(\"id\") " + bean.idType + " id) {");
	...
	private void createDeleteOneToOneComponent() {
		for (OneToOneComponent oneToOneComponent : this.bean.oneToOneComponentList) {
			...
			writeLine("public void delete" + currentBean.className + "(@PathVariable(\"id\")" + bean.idType + " id) {");			
	...
	private void createDeleteOneToManyComponent() {
		for (OneToManyComponent oneToManyComponent : this.bean.oneToManyComponentList) {
			...
			writeLine(" */");
			writeLine("@RequestMapping(value = {" + bean.serviceInterfaceName + ".DELETE_" + currentBean.table.originalName + "_URL}, method = RequestMethod.DELETE)");
			writeLine("public void delete" + currentBean.className + "(@PathVariable(\"id\")" + currentBean.idType + " id) {");			
			writeLine(bean.serviceObjectName + ".delete" + currentBean.className + "(id);");
			writeLine("}");
			skipLine();
		}
