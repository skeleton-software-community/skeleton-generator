...
private Bean bean;
	...
	private void createUpdateObject() {
		...
		writeLine("public void update(@PathVariable(\"id\") " + bean.idType + " id, @Valid @RequestBody " + this.bean.formBean.className + " form) {");
	...
	private void createUpdateUniqueComponent() {
		for (OneToOneComponent oneToOneComponent : this.bean.oneToOneComponentList) {
			...
			writeLine("public void update" + currentBean.className + "(" + bean.idType + " id, @Valid @RequestBody " + currentBean.formBean.className + " form) {");
	...
	private void createUpdateOneToManyComponent() {
		for (OneToManyComponent oneToManyComponent : this.bean.oneToManyComponentList) {
			...
			writeLine(" */");
			writeLine("@RequestMapping(value = {" + bean.serviceInterfaceName + ".UPDATE_" + currentBean.table.originalName + "_URL}, method = RequestMethod.PUT)");
			writeLine("public void update" + currentBean.className + "(" + currentBean.idType + " id, @Valid @RequestBody " + currentBean.formBean.className + " form) {");
			writeLine(bean.serviceObjectName + ".update" + currentBean.className + "(id, form);");
			writeLine("}");
			skipLine();
		}
