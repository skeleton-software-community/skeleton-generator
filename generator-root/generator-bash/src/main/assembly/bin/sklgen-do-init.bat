...
:PROMPT_SKL_TYPE
set /p SKELETON_NUMBER=Enter your skeleton type (1:SPRING_HIBERNATE_RICHFACES_4, 2:SPRING_HIBERNATE_RICHFACES_3) : 
if %SKELETON_NUMBER% == 2 (set SKELETON_TYPE=SPRING_HIBERNATE_RICHFACES_3& goto PROMPT_SKL_TYPE_OK)
if %SKELETON_NUMBER% == 1 (set SKELETON_TYPE=SPRING_HIBERNATE_RICHFACES_4& goto PROMPT_SKL_TYPE_OK)

goto PROMPT_SKL_TYPE
:PROMPT_SKL_TYPE_OK

:PROMPT_DATABASE_ENGINE
set /p DATABASE_ENGINE_NUMBER=Enter your database engine (1:POSTGRESQL, 2:ORACLE) : 
if %DATABASE_ENGINE_NUMBER% == 2 (set DATABASE_ENGINE=ORACLE& goto PROMPT_DATABASE_ENGINE_OK)
if %DATABASE_ENGINE_NUMBER% == 1 (set DATABASE_ENGINE=POSTGRESQL& goto PROMPT_DATABASE_ENGINE_OK)
